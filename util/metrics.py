...
def calculate_IoU_Per_Epoch(model,val_inputs_path,val_masks_path,checkpoint_path,epoch_number,height=256, width=256, one_hot_label=False, wavelet=True, change=False):
...
#                 [s for s in val_samples if "daegu" in s]
    val_samples_mini = val_samples[0:] #+ val_samples[2000:2015]
...
#         print(img)
        ...
        gt =  data_util.get_mask(val_masks_path+img,one_hot_label=one_hot_label,height=height, width=width, do_aug=[])[:,:,0].astype(int)
        input_image = data_util.get_image(val_inputs_path+img,do_aug=[], width=width,change=change)
        if wavelet:
            input_image_gray = data_util.get_image(val_inputs_path+img,do_aug=[],gray=True, width=width,change=change)
            input_image_gray = input_image_gray if not change else input_image_gray[:, :width]
            ...
            if change:
                input0, input1 = input_image[:, :width], input_image[:, width:]
                pred = model.predict([np.expand_dims(input0, axis=0),np.expand_dims(input1, axis=0), w1, w2, w3, w4], batch_size=None, verbose=0, steps=None)
            else:
