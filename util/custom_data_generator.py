...
def get_image(path, do_aug, gray=False, width=256, change=False):
    if gray:
    ...
    #     img2 = img[:, width:, :]
        ...
        #low1 = cv2.filter2D(img1, -1, kernel_sharp)
        #img1 = cv2.bilateralFilter(low1, 3, 75, 75)

        #low2 = cv2.filter2D(img2, -1, kernel_sharp)
        #img2 = cv2.bilateralFilter(low2, 3, 75, 75)
...
def get_mask(path,one_hot_label, height, width, do_aug=False):
...
#             label[: , : , c ] = (mask == c ).astype(int)
        label = np.reshape(label, (height * width, 2)).astype(int)
    else:
        label= np.expand_dims(mask,axis=-1)
    return label

def image_generator(files,images_path,masks_path, batch_size = 5, wavelet=True, change=False, height=256, width=256, one_hot_label = False, data_aug= False):
    ...
    while True:
        ...
        for i in range(batch_size):
            ...
            input = get_image(images_path + file_path,do_aug, width=width, change=change)
            output = get_mask(masks_path + file_path,one_hot_label, height=height, width=width, do_aug=do_aug)

            if wavelet:
                input_gray = get_image(images_path + file_path, do_aug=do_aug, width=width, gray=True)
                input_gray = input_gray if not change else input_gray[:, :width]
                coeffs = lanenet_wavelet(input_gray)
                w1, w2, w3, w4 = coeffs
                waves1.append(w1)
                waves2.append(w2)
                waves3.append(w3)
                waves4.append(w4)
            if change:
                batch_input.append(input[:, :width,])
                batch_input2.append(input[:, width:,])
            else:
        ...
        if change and wavelet:
            yield ([batch_x, batch_x2, waves1, waves2, waves3, waves4], batch_y)
        elif wavelet:
