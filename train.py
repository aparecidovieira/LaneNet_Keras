...
models_wavelet = ['lanenet', 'lanenet2', 'lanenetRes', 'linenet']
wavelet = True if model_name in models_wavelet else False
change = True if (model_name == 'lanenet2' or model_name == 'linenet' or  model_name == 'lanenet2' or model_name == 'linenet2') else False
...
print('XXxxXX', change, model_name)
train_generator = custom_data_generator.image_generator(train_samples,train_inputs_path,train_masks_path, batch_size,one_hot_label=one_hot_label,data_aug=data_aug, height=height, width=width, change=change, wavelet=wavelet)
val_generator = custom_data_generator.image_generator(val_samples,val_inputs_path,val_masks_path,batch_size,one_hot_label=one_hot_label, change=change, height=height, width=width, wavelet=wavelet)
...
if args.continue_training:
    ...
    weights = sorted(glob.glob(checkpoint_dir + "lanenet_weights_" + '*'))[-1]
...
class onEachEpochCheckPoint(Callback):
    def __init__(self, model_parallel, path,model, change=change, one_hot_label=one_hot_label, height=height, width=width, wavelet=wavelet):
        super().__init__()
        self.path = path
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        BG_IU, BD_IU, BG_P, BD_P = metrics.calculate_IoU_Per_Epoch(model,val_inputs_path,val_masks_path,checkpoint_dir,epoch,change=self.change, one_hot_label=self.one_hot_label, height=self.height, width=self.width, wavelet=self.wavelet)
...
model_checkpoint = onEachEpochCheckPoint(model_parallel, weights_path, model,one_hot_label=one_hot_label,change=change, height=height, width=width, wavelet=wavelet)
